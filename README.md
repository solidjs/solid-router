...
- [`liveQuery(fn, key)`](#livequery-experimental): the frame stream stays open and the channel owns it — hover connects it (held through the preload window, so a hovered link is an open stream), `revalidate(key)` reconnects, and the mutation sweep leaves it alone since the stream is its own freshness, and the collector takes the producer's first yield so the mutation response pushes current state into the open channel.
...
- **Single-flight** — the collector invokes the producer for its first yield (current state, by contract) and closes it there, so the mutation's flight payload pushes current state straight into the open channel: mutated state lands in the same round trip, and the stream stays authoritative for everything after.
